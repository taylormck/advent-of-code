...
main :: proc() {
    ...
    for update in updates {
        if !is_update_valid(update, orderings[:]) {
            update_fix_order(update, orderings[:])
...
is_ordering_respected :: proc(update: Update, ordering: Ordering) -> bool {
    ...
    for page_number in update {
        if page_number == ordering.second {
            second_found = true
        } else if second_found && page_number == ordering.first {
            return false
        }
    }
...
update_fix_order :: proc(update: Update, orderings: []Ordering) {
    for !is_update_valid(update, orderings) {
        for ordering in orderings {
            update_swap(update, ordering)
        }
    }
}

update_swap :: proc(update: Update, ordering: Ordering) {
    second_found_index := -1
    for page_num, i in update {
        if page_num == ordering.second {
            second_found_index = i
        } else if page_num == ordering.first {
            if second_found_index < 0 {
                break
            }

            update[i] = update[second_found_index]
            update[second_found_index] = page_num
            break
        }
    }
}
